...
Class Radify {
    ...
    ********************************************************************************************/
    ...
    static RenderMenu(oMenu)
    {
        ...
        if (oMenu.options.enableDarkening)
            this.CreateDarkeningGui(oMenu)
    ...
    static CreateDarkeningGui(oMenu)
    {
        darkeningSize := Round(oMenu.options.itemSize * oMenu.dpiScale)
        g := Gui('-Caption +E0x80000 +E0x08000000 +E0x20 +Owner' oMenu.hwnd, 'RadifyDarkeningGUI_' oMenu.id)
        g.Show('Hide')
        oMenu.darkeningGui := g
        oMenu.darkeningHwnd := g.Hwnd
        oMenu.darkeningHdc := CreateCompatibleDC()
        oMenu.darkeningHbm := CreateDIBSection(darkeningSize, darkeningSize)
        oMenu.darkeningObm := SelectObject(oMenu.darkeningHdc, oMenu.darkeningHbm)
        oMenu.darkeningG := Gdip_GraphicsFromHDC(oMenu.darkeningHdc)
        Gdip_SetSmoothingMode(oMenu.darkeningG, oMenu.options.smoothingMode)
        Gdip_SetInterpolationMode(oMenu.darkeningG, oMenu.options.interpolationMode)
        Gdip_GraphicsClear(oMenu.darkeningG, 0x00FFFFFF)
        pBrush := Gdip_BrushCreateSolid(0x40000000)
        Gdip_FillEllipse(oMenu.darkeningG, pBrush, 0, 0, darkeningSize, darkeningSize)
        Gdip_DeleteBrush(pBrush)
        UpdateLayeredWindow(oMenu.darkeningHwnd, oMenu.darkeningHdc, 0, 0, darkeningSize, darkeningSize)
    }
    ...
    static UpdateMenuGlowTooltip(oMenu, *)
    {
        ...
        if (itemInfo := this.FindHoveredItem(oMenu, mouseInfo.mouseX, mouseInfo.mouseY)) {
            if (oMenu.prevHoveredIndex != itemInfo.index) {
                ...
                if (oMenu.options.enableDarkening) {
                    darkeningX := Round(itemInfo.winX + itemInfo.item.absX * oMenu.dpiScale)
                    darkeningY := Round(itemInfo.winY + itemInfo.item.absY * oMenu.dpiScale)
                    darkeningSize := Round(itemInfo.item.itemClickZoneSize * oMenu.dpiScale)
                    oMenu.darkeningGui.Show('x' darkeningX ' y' darkeningY ' w' darkeningSize ' h' darkeningSize ' NA')
                }
    ...
    ********************************************************************************************/
    ...
    static DisposeMenuResources(oMenu)
    {
        ...
        if (oMenu.HasOwnProp('darkeningG') && oMenu.darkeningG) {
            Gdip_DeleteGraphics(oMenu.darkeningG)
            oMenu.darkeningG := 0
        }

        if (oMenu.HasOwnProp('darkeningHdc') && oMenu.darkeningHdc) {
            SelectObject(oMenu.darkeningHdc, oMenu.darkeningObm)
            DeleteObject(oMenu.darkeningHbm)
            DeleteDC(oMenu.darkeningHdc)
            oMenu.darkeningHdc := 0
            oMenu.darkeningHbm := 0
        }
        ...
        if (oMenu.HasOwnProp('darkeningGui') && oMenu.darkeningGui) {
            oMenu.darkeningGui.Destroy()
            oMenu.darkeningGui := 0
        }
    }
...
/*************************************************************************************************
 * @description: JSON????�r�Ŧ�??????????��???? �ק�??[HotKeyIt/Yaml](https://github.com/HotKeyIt/Yaml)
 * �W??�F?true/false/null?????��?? �O??�F????????
